function fruitChatApp() {
  ...
  function closeModal(id) {
    ...
    // тайминг под твой CSS transition (обычно 300мс)
  ...
  return {
    ...
    // Facts/topics (оставил твои массивы — можно хранить как есть)
    interestingFacts: [], // заполним из файла ниже в init()
    topics: [],           // заполним из файла ниже в init()
    ...
    // ===== Init =====
    async init() {
      // 1) Данные (берём из текущего app.js “битого”, но здесь зададим корректно)
    ...
    confirmClearAllChats() {
      ...
      // создаём новый чат, чтобы UI не остался без currentChat
      this.createNewChat();
    ...
    addMessage(role, content, opts = {}) {
      ...
      // empty-state у тебя через x-show, поэтому не удаляем DOM-элемент
    ...
    renderMessages() {
      ...
      // быстрый рендер: без scroll/katex на каждую строку
...
Твой характер: добрый, умный, терпеливый, с чувством юмора.`
    ...
    setupEventListeners() {
      ...
      // клики по overlay уже обрабатываются Alpine (@click в HTML),
      // но если пользователь кликнет по пустоте — тоже закроем
    ...
    // ===== Data providers (твои списки, чтобы ничего не терять) =====
    _getFacts() {
      // Взял из твоего текущего файла (привёл к корректному JS)
      return [
    ...
    _getTopics() {
      // Полный список тем можно оставить как был — здесь оставлю всё твое содержимое “как есть”
      return [
        'Дроби и их сложение',
